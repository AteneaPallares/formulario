  ...
  while ($row=mysqli_fetch_row($result))
    {
      ...
      $seguir=true;
        ...
        if($comparar!=$row[1] && $seguir==true){
            ...
            $i=0;
            $arreglo[$i]=$comparar;
            $nuevo="SELECT ID,NUMERO,PROYECTO,FECHA,OBSERVACIONES,FECHADOS,ESTATUS1,DISENADOR FROM datos WHERE NUMERO=$arreglo[$i] ORDER BY FECHADOS";
            if($rep=mysqli_query($link,$nuevo)){
              $filtrobool=false;
                while($sele=mysqli_fetch_row($rep)){
                  
                  if(($sele[6]==$filtro or $filtro=='7') &&($sele[7]==$username || $username=="admin")){
                    $filtrobool=true;
                    $aux=$sele[0];
                    $nomb=$sele[2];
                    $numero=$sele[1];
